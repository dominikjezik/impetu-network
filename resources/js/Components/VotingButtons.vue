<template>
    <div class="voting-buttons">
        <button class="btn-vote btn-vote-up" @click.prevent="upvote" :class="{ 'btn-vote-up-clicked' : isUpvoted}">
        ...
        <span class="score">{{ voteable.score }}</span>
        <button class="btn-vote btn-vote-down" @click.prevent="downvote" :class="{ 'btn-vote-down-clicked' : isDownvoted}">
            <svg width="410" height="492" viewBox="0 0 410 492" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
    ...
    computed: {
        isUpvoted() {
            return this.voteable.voted_by_user === null ? false : this.voteable.voted_by_user.upvote === 1
        },
        isDownvoted() {
            return this.voteable.voted_by_user === null ? false : this.voteable.voted_by_user.upvote === 0
        }
    },
    methods: {
        upvote() {
            axios.post(`${this.endpoint}/upvote`)
                .then(res => { this.updateVote(true) })
                .catch(err => console.log(err))
        },
        downvote() {
            axios.post(`${this.endpoint}/downvote`)
                .then(res => { this.updateVote(false) })
                .catch(err => console.log(err))
        },
        updateVote(upvote) {
            if(this.voteable.voted_by_user == null) {
                this.voteable.voted_by_user = {
                    upvote: null
                }
            }
            if(upvote) {
                if(this.voteable.voted_by_user.upvote === 1) {
                    this.voteable.voted_by_user.upvote = null
                    this.voteable.score--
                } else {


                    if(this.voteable.voted_by_user.upvote === 0) {
                        this.voteable.score += 2
                    }
                    else {
                        this.voteable.score++
                    }
                    this.voteable.voted_by_user.upvote = 1
                }
            } else {
                if(this.voteable.voted_by_user.upvote === 0) {
                    this.voteable.voted_by_user.upvote = null
                    ...
                    this.voteable.voted_by_user.upvote = 0
                }
            }
        }
    }
